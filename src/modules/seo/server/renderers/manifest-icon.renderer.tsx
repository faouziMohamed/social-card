...
export function manifestIconRenderer(p: ManifestIconParams, ctx: OgRendererContext): React.ReactElement {
  ...
  const rx =
    p.shape === 'circle'  ? width / 2 :
    p.shape === 'rounded' ? Math.round(width * 0.15) : 0;
  ...
  return (
    <div style={{ display: 'flex', width, height, borderRadius: rx, backgroundColor: bg, position: 'relative', overflow: 'hidden' }}>
      ...
      {p.logo
        ? <img src={p.logo} style={{ width: logoSize, height: logoSize, margin: 'auto', objectFit: 'contain', position: 'relative' }} />
        : <span style={{ margin: 'auto', fontSize: textSize, fontWeight: 700, color: fg,
            fontFamily: 'system-ui,-apple-system,sans-serif', lineHeight: 1, letterSpacing: '-0.02em', position: 'relative' }}>
      ...
      <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, display: 'flex',
        height: Math.round(width * 0.03), backgroundColor: accent, opacity: 0.6 }} />
    </div>
  );
}
