...
export const iconBaseSchema = z.object({
  initial:     z.string().max(2).default('A').describe('1–2 letter monogram shown when no logo is provided'),
  logo:        z.string().url().optional().describe('Absolute URL of a logo/icon image — renders instead of monogram'),
  color:       hex.default('#0f0f0f').describe('Background color (hex)'),
  accentColor: hex.default('#6366f1').describe('Foreground / ring accent color (hex)'),
  shape:       z.enum(['circle', 'square', 'rounded']).default('rounded').describe("Icon shape: 'circle' | 'square' | 'rounded'"),
...
export const manifestIconSchema = iconBaseSchema.extend({
  size: z.enum(['192', '512']).default('512').describe('Icon size: 192 or 512 px'),
});
...
export const twitterCardSchema = z.object({
  ...
  theme:       z.enum(['dark', 'light']).default('dark').describe("Color theme: 'dark' | 'light'"),
  bgStyle:     z.string().default('gradient+grid').describe("Background style tokens e.g. 'gradient+grid'"),
});
