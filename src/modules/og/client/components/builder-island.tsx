      ...
      <section className="rounded-2xl border border-border/30 bg-card/20 pb-2 pt-4 px-2">
        <div className="mb-1 px-2 flex items-center justify-between">
          ...
          <span className="text-[10px] text-muted-fg/35">
            {OG_TEMPLATE_META.findIndex((t) => t.name === template) + 1} / {OG_TEMPLATE_META.length}
          </span>
        </div>
        <TemplateSelector meta={OG_TEMPLATE_META} value={template} onChange={(t) => setTemplate(t as typeof template)} />
      </section>
...
function BadgeBuilder() {
  const {
    template, params, previewUrl, badgeUrl,
    setTemplate, setParam, resetParams,
    ...
    <div className="flex min-w-0 flex-col gap-6">
      ...
      <section className="rounded-2xl border border-border/30 bg-card/20 pb-2 pt-4 px-2">
        <div className="mb-1 px-2 flex items-center justify-between">
          ...
          <span className="text-[10px] text-muted-fg/35">
            {BADGE_TEMPLATE_META.findIndex((t) => t.name === template) + 1} / {BADGE_TEMPLATE_META.length}
          </span>
        </div>
        <TemplateSelector meta={BADGE_TEMPLATE_META} value={template} onChange={(t) => setTemplate(t as typeof template)} />
      </section>
      ...
      <div className="flex min-w-0 flex-col gap-6 lg:grid lg:grid-cols-[380px_minmax(0,1fr)] lg:items-start lg:gap-8">
        ...
        <div className="flex min-w-0 flex-col gap-4">
          ...
          <div className="flex items-center justify-start rounded-xl border border-border/30 bg-card/20 p-4">
            <PreviewPanel src={previewUrl} aspectRatio={badgeAspect} className="max-w-[280px]" />
          </div>
...
function SeoBuilder() {
  const {
    template, params, previewUrl, seoUrl,
    setTemplate, setParam, resetParams,
  } = useSeoBuilderState();
    ...
    <div className="flex min-w-0 flex-col gap-6">
      ...
      <section className="rounded-2xl border border-border/30 bg-card/20 pb-2 pt-4 px-2">
        <div className="mb-1 px-2 flex items-center justify-between">
          ...
          <span className="text-[10px] text-muted-fg/35">
            {SEO_TEMPLATE_META.findIndex((t) => t.name === template) + 1} / {SEO_TEMPLATE_META.length}
          </span>
        </div>
        <TemplateSelector meta={SEO_TEMPLATE_META} value={template} onChange={(t) => setTemplate(t as typeof template)} />
      </section>
            ...
            <div className="mb-2 flex items-center justify-between">
              ...
              >
                {snippetCopied
                  ...
                  : <><Copy className="h-3 w-3 opacity-50" /><span>Copy</span></>
                }
              </Button>
            </div>
