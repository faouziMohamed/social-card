...
export function PreviewPanel({ src, aspectRatio = "1200/630", className }: PreviewPanelProps) {
  ...
  return (
    ...
    >
      {isLoading && <div className="absolute inset-0 shimmer-track rounded-xl" />}

      {hasError && (
        <div className="absolute inset-0 flex flex-col items-center justify-center gap-3">
          <span className="text-3xl opacity-40">⚠</span>
          <p className="text-xs text-muted-fg/50 font-mono">Preview unavailable</p>
        </div>
      )}
      ...
      {isLoading && (
        <div className="absolute right-3 top-3 flex items-center gap-1.5 rounded-full bg-black/60 px-2.5 py-1 backdrop-blur-sm">
          ...
          <span className="text-[9px] font-bold tracking-widest uppercase text-muted-fg">Rendering</span>
        </div>
      )}
      {loaded && (
        <div className="absolute right-3 top-3 flex items-center gap-1.5 rounded-full bg-black/60 px-2.5 py-1 backdrop-blur-sm opacity-0 hover:opacity-100 transition-opacity">
          ...
          <span className="text-[9px] font-bold tracking-widest uppercase text-terminal-green">Live</span>
        </div>
      )}
    </div>
  );
