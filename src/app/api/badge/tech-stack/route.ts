...
export function techStackRenderer(p: TechStackParams): string {
  ...
  const gap = 6;
  const outerPad = 7;
  const widths = safeTags.map(
    tag => estimateTextWidth(tag, fontSize) + tagPadding * 2,
  );

  if (isRow) {
    const tagsWidth =
      widths.reduce((sum, current) => sum + current, 0) +
      gap * Math.max(safeTags.length - 1, 0);
  ...
  const tagElements = rows.map(({tag, width, x, row}, index) => {
    ...
    return [
      ...
      text(
        tag,
        outerPad + x + tagPadding,
        y + tagHeight / 2,
        hexToRgba(accent, p.theme === 'dark' ? 0.96 : 0.9),
        fontSize,
        'bold',
      ),
    ].join('');
